...
- **Logging**: SLF4J, structured, always include `jobId` (and `segmentId`/`rung` in workers) — bound
  via the `AbstractStageWorker.mdcContext` MDC hook and surfaced by the worker profiles' log pattern.
