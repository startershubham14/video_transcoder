...
A distributed video-processing service that ingests a video, splits it into keyframe-aligned
segments, transcodes them **in parallel** across a scalable worker pool, and packages the result
into **MP4 and adaptive-bitrate HLS**. FFmpeg does the transcoding — the focus of the project is the
**distributed job-processing system around it**: an async job queue, fan-out/fan-in coordination,
idempotent retries with a dead-letter queue, backpressure, live status, and horizontal scaling.
Functional end-to-end locally via `docker compose up`; `./mvnw verify` is green (unit + Testcontainers).

**Stack:** Java 21 · Spring Boot (Spring Web + Spring AMQP) · RabbitMQ · PostgreSQL (Spring Data JPA) ·
AWS S3 (SDK v2; MinIO locally) · ClamAV · Docker / docker-compose. Workers are `@RabbitListener`
consumers scaled by replica count.
...
| **Graceful shutdown** — never drop in-flight work on scale-down | Spring graceful shutdown + listener stops before DB/S3 clients; compose `stop_grace_period` | [`RabbitMqConfig`](src/main/java/dev/shubham/transcoder/config/RabbitMqConfig.java), [`docker-compose.yml`](docker-compose.yml) | manual e2e |
...
├── docker-compose.bench.yml  # scaling-benchmark overlay (caps each transcode worker's CPU)
