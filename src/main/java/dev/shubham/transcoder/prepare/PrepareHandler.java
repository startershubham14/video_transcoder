...
/**
 * Stage-1 orchestration + state: download source → ffprobe (validity, real duration/size) →
 ...
 * {@link BlobStore}) and the {@link LadderPolicy}, never the vendor tools directly. Any guard
 * step failing transitions the job to {@code FAILED} with a reason. Transport is handled by
 * {@link PrepareListener}.
 */
@Service
