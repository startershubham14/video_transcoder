...
/**
 * FFmpeg adapter for {@link Splitter}. Splits the source into <em>keyframe-aligned</em>
 * segments — the real parallel-processing unit. A segment is only independently
 * transcodable if it starts on a keyframe (GOP boundary), so the target length is approximate.
 *
